...
# 2006-12-02 fixed a bug in looking up entries from /etc/passwd
...
sub traceaddr($ ) {
  ...
  if (not defined($st)) {
    ...
    print('empty .qmail file: "', $qme,
          "\"\ndeliver according to default delivery instructions\n");
    return;
  } else {
